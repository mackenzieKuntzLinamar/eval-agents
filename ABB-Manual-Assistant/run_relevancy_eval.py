...
responses with the RAGAS Answer Relevancy metric.

Answer Relevancy (``answer_relevancy``): how relevant the agent's answer is to
the user's question. Uses an LLM + embeddings.

A score is uploaded only when:
1. The ABB agent returns a non-empty answer without an error.
2. The metric produced a valid numeric value.
3. A Langfuse trace ID is available.

Usage:
    python run_relevancy_eval.py \
        --langfuse_dataset_name "SME Test Set - Formatted" \
        --run_name "Relevancy_Eval_Run_01" \
        --limit 10
"""
...
# so the defaults below are Gemini models rather than OpenAI ones.
...
# The RAGAS LLM default output cap is only 1024 tokens. gemini-2.5-flash is a
# reasoning model, so hidden reasoning tokens share the output budget -- give it
# plenty of headroom so the evaluation output is not truncated for long answers.
...
async def score_answer_relevancy(
    ...
    """Compute the RAGAS Answer Relevancy score for one question/answer pair."""
    try:
        ...
        print("\n[RAGAS] ANSWER RELEVANCY FAILED", flush=True)
...
async def run_and_evaluate(
    ...
    value, reason = await score_answer_relevancy(query, answer_for_scoring)

    print(f"Answer Relevancy: {value}", flush=True)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run ABB Manual Assistant RAGAS answer relevancy evaluations from a "
            "Langfuse dataset."
        )
    )
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "Defaults to 'answer_relevancy_<run_name>.csv'."
        ),
    )
...
async def main() -> None:
    ...
    print(f"\nUploaded {uploads} answer_relevancy score(s).", flush=True)
    ...
    write_scores_file(output_file, "answer_relevancy", results)
