...
async def run_agent_with_trace(orchestrator: Orchestrator, query: str) -> LangFuseTracedResponse:
    ...
    return LangFuseTracedResponse(answer=answer, trace_id=langfuse_client.get_current_trace_id())
...
async def run_and_evaluate(run_name: str, orchestrator: Orchestrator, item: DatasetItemClient):
    ...
    evaluator_response = await run_evaluator_agent(
        EvaluatorQuery(
            question=item.input["text"], ground_truth=expected_output["text"], proposed_response=traced_response.answer
        )
    )
